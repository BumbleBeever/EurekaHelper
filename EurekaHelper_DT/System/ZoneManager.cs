    ...
    public class ZoneManager
    {
        ...
        private nint InitZoneDetour(nint a1, int a2, nint a3)
        {
            try
            {
                ...
                if (zoneName != null)
                {
                    if (EurekaHelper_DT.Config.DisplayServerId)
                        EurekaHelper_DT.PrintMessage($"{zoneName} Server ID: {serverId}");

                    if (EurekaHelper_DT.Config.DisplayServerIdInServerInfo)
                    {
