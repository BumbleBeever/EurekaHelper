...
namespace EurekaHelper_DT.System
{
    ...
    public class AlarmManager : IDisposable
    {
        ...
        public void AddAlarm(EurekaAlarm alarm)
        {
            ...
            EurekaHelper_DT.Config.Alarms.Add(alarm);
        ...
        public void DeleteAlarm(EurekaAlarm alarm, bool deleteAll = false)
        {
            if (deleteAll)
            {
                EurekaHelper_DT.Config.Alarms.Clear();
                EurekaHelper_DT.Config.Save();
            ...
            EurekaHelper_DT.Config.Alarms.RemoveAll(x => x.ID == alarm.ID);
            EurekaHelper_DT.Config.Save();
        ...
        private void GetActiveAlarms()
        {
            ...
            foreach (var alarm in EurekaHelper_DT.Config.Alarms.Where(x => x.Enabled))
                AddActiveAlarm(alarm);
        }
