...
using EurekaHelper_DT.XIV.Zones;
using System;
    ...
    public class ElementalManager : IDisposable
    {
        ...
        private void OnTerritoryChanged(ushort territoryId)
        {
            if (Utils.IsPlayerInEurekaZone(territoryId))
            {
                ...
                if (EurekaHelper_DT.Config.ElementalAlwaysClear)
                    Elementals.Clear();
            }
            else
            {
        ...
        {
            if (!EurekaHelper_DT.Config.ElementalCrowdsource && !EurekaHelper_DT.Config.DisplayElemental && !EurekaHelper_DT.Config.DisplayElementalToast)
                return;
            ...
            foreach (var elemental in elementals)
            {
                ...
                if (EurekaHelper_DT.Config.ElementalCrowdsource)
                {
                    ...
                    {
                        EurekaHelper_DT.PrintMessage("Elemental found that is not in the plugin database.");
                        EurekaHelper_DT.PrintMessage("Please send the following information to the developer on GitHub or Discord DM. You can find the contact information in the \"About\" tab.");
                        EurekaHelper_DT.PrintMessage("You can also opt-out of crowdsourcing for Elemental positions in the \"Elementals\" tab.");
                        EurekaHelper_DT.PrintMessage($"Send -> T: {DalamudApi.ClientState.TerritoryType} X: {eurekaElemental.RawPosition.X} Y: {eurekaElemental.RawPosition.Y} Z: {eurekaElemental.RawPosition.Z}");
                    }
                }
                ...
                if (EurekaHelper_DT.Config.DisplayElementalToast)
                    DalamudApi.ToastGui.ShowQuest(sb.BuiltString);

                if (EurekaHelper_DT.Config.DisplayElemental)
                {
                    DalamudApi.PluginInterface.RemoveChatLinkHandler(eurekaElemental.ObjectId);

                    if (EurekaHelper_DT.Config.ElementalPayloadOptions != PayloadOptions.Nothing)
                    {
                        ...
                        var text = EurekaHelper_DT.Config.ElementalPayloadOptions switch
                        {
                ...
                if (EurekaHelper_DT.Config.ElementalAutoMark)
                {
