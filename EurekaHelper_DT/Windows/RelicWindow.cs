...
using EurekaHelper_DT.XIV.Relic;
...
namespace EurekaHelper_DT.Windows
{
    internal class RelicWindow : Window, IDisposable
    {
        ...
        public RelicWindow(EurekaHelper_DT plugin) : base("Eureka Helper - Relic")
        {
            Plugin = plugin;
                    ...
                    if (ImGui.BeginTabItem(stage.Key))
                    {
                        ...
                        foreach (var requirement in stage.Value.CompletionRequirements)
                        {
                            ...
                            if (requirement.ItemId != 0)
                            {
                                if (ImGui.IsItemClicked())
                                {
                                    ...
                                    EurekaHelper_DT.PrintMessage(itemLink);
                                }
                            }
                        ...
                        if (ImGui.BeginTable("ItemsDisplayWindow", 3, ImGuiTableFlags.Resizable | ImGuiTableFlags.BordersInnerH | ImGuiTableFlags.BordersV | ImGuiTableFlags.NoBordersInBody | ImGuiTableFlags.ScrollY | ImGuiTableFlags.NoSavedSettings | ImGuiTableFlags.Sortable | ImGuiTableFlags.SortTristate))
                        {
                            ...
                            foreach (var relic in currentStageRelics)
                            {
                                ...
                                var relicCompleted = EurekaHelper_DT.Config.CompletedRelics.Contains(relic.ItemId);
                                ...
                                if (!relicCompleted)
                                {
                                    ...
                                    {
                                        EurekaHelper_DT.Config.CompletedRelics.Add(relic.ItemId);
                                ...
                                else
                                {
                                    ...
                                    {
                                        EurekaHelper_DT.Config.CompletedRelics.Remove(relic.ItemId);
                                        EurekaHelper_DT.Config.Save();
                                    }
                                }
                                ImGui.PopFont();
