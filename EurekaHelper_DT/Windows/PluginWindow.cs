...
namespace EurekaHelper_DT.Windows
{
    public class PluginWindow : Window, IDisposable
    {
        private readonly EurekaHelper_DT Plugin = null!;

        public PluginWindow(EurekaHelper_DT plugin) : base("Eureka Helper")
        {
            Plugin = plugin;
                ...
                if (Connection.CanModify())
                {
                    ...
                    else
                    {
                        if (ImGuiComponents.IconButton(FontAwesomeIcon.LockOpen))
                        {
                            ...
                            if (datacenterId == 0)
                                EurekaHelper_DT.PrintMessage("This datacenter is not supported currently. Please submit an issue if you think this is incorrect.");
                            else
        ...
        public async Task CreateTracker(int zoneId, bool printMessage = false)
        {
            ...
            if (printMessage)
                EurekaHelper_DT.PrintMessage($"Successfully created a tracker: {Utils.CombineUrl(Constants.EurekaTrackerLink, trackerId)}");
        }
        ...
        public async Task ExportTracker(string oldTrackerId, bool printMessage = false)
        {
            ...
            if (printMessage)
                EurekaHelper_DT.PrintMessage($"Successfully exported the previous tracker: {Utils.CombineUrl(Constants.EurekaTrackerLink, trackerId)}");
        }
        ...
        public void DrawTrackerTable()
        {
            ...
            if (Connection.IsConnected())
            {
                ...
                if (ImGui.BeginTable("TrackerTable", numColumns, ImGuiTableFlags.Resizable | ImGuiTableFlags.BordersInnerH | ImGuiTableFlags.BordersV | ImGuiTableFlags.NoBordersInBody | ImGuiTableFlags.ScrollY | ImGuiTableFlags.NoSavedSettings | ImGuiTableFlags.Sortable | ImGuiTableFlags.SortTristate))
                {
                    ...
                    if (!EurekaHelper_DT.Config.ShowLevelInTrackerTable)
        ...
        public void DrawElementalTab()
        {
            ...
            save |= ImGui.Checkbox("Display Elemental", ref EurekaHelper_DT.Config.DisplayElemental);
            Utils.SetTooltip("Displays in chat whenever an Elemental appears near the player");
            ImGui.NextColumn();

            save |= ImGui.Checkbox("Display Elemental Toast", ref EurekaHelper_DT.Config.DisplayElementalToast);
            Utils.SetTooltip("Displays a toast whenever an Elemental appears near the player");
            ImGui.NextColumn();

            save |= ImGui.Checkbox("Crowdsource Locations", ref EurekaHelper_DT.Config.ElementalCrowdsource);
            ...
            var enumCurrent = Array.IndexOf(enumValues, EurekaHelper_DT.Config.ElementalPayloadOptions);
            if (ImGui.Combo("Payload Options", ref enumCurrent, enumNames, enumNames.Length))
            {
                EurekaHelper_DT.Config.ElementalPayloadOptions = enumValues[enumCurrent];
                save = true;
            }
            ...
            save |= ImGui.Checkbox("Auto Mark Elementals", ref EurekaHelper_DT.Config.ElementalAutoMark);
            ...
            save |= ImGui.Checkbox("Always Clear Elementals", ref EurekaHelper_DT.Config.ElementalAlwaysClear);
            ...
            if (ImGui.Button("Add Known Elemental Map Markers"))
            {
                ...
                {
                    EurekaHelper_DT.PrintMessage("You must be in one of the Eureka zone to use this.");
                }
            }
            ...
            if (ImGui.BeginTable("ElementalsTable", 6, ImGuiTableFlags.Resizable | ImGuiTableFlags.BordersInnerH | ImGuiTableFlags.BordersV | ImGuiTableFlags.NoBordersInBody | ImGuiTableFlags.ScrollY | ImGuiTableFlags.NoSavedSettings))
            {
                ...
                for (int i = Plugin.ElementalManager.Elementals.Count - 1; i >= 0; i--)
                {
                    ...
                    ImGui.TableNextColumn();
                    if (ImGui.Button($"{(EurekaHelper_DT.Config.ElementalPayloadOptions == PayloadOptions.CopyToClipboard ? $"C##{elemental.ObjectId}" : $"S##{elemental.ObjectId}")}"))
                    {
                        Utils.SetFlagMarker(elemental.TerritoryId, elemental.MapId, elemental.Position);
                        switch (EurekaHelper_DT.Config.ElementalPayloadOptions)
                        {
        ...
        public static void DrawSettingsTab()
        {
            ...
            save |= ImGui.Checkbox("Display NM Pop", ref EurekaHelper_DT.Config.DisplayFatePop);
            Utils.SetTooltip("Displays the NM that popped in chat");
            ImGui.NextColumn();

            save |= ImGui.Checkbox("Enable NM pop sound", ref EurekaHelper_DT.Config.PlayPopSound);
            Utils.SetTooltip("A sound que will be played whenever an NM pops.");
            ImGui.NextColumn();

            save |= ImGui.Checkbox("Display fate progress", ref EurekaHelper_DT.Config.DisplayFateProgress);
            Utils.SetTooltip("Prints the NM progress in chat");
            ImGui.NextColumn();

            save |= ImGui.Checkbox("Enable bunny fates", ref EurekaHelper_DT.Config.DisplayBunnyFates);
            Utils.SetTooltip("Enable display for bunny fates");
            ImGui.NextColumn();

            save |= ImGui.Checkbox("Display Toast", ref EurekaHelper_DT.Config.DisplayToastPop);
            Utils.SetTooltip("Displays a toast whenever an NM pops");
            ImGui.NextColumn();

            save |= ImGui.Checkbox("Auto pop fate", ref EurekaHelper_DT.Config.AutoPopFate);
            ...
            var enumCurrent = Array.IndexOf(enumValues, EurekaHelper_DT.Config.NMSoundEffect);
            if (ImGui.Combo("NM Sound Effect", ref enumCurrent, enumNames, enumNames.Length))
            {
                EurekaHelper_DT.Config.NMSoundEffect = enumValues[enumCurrent];
            ...
            ImGui.SetNextItemWidth(140f);
            enumCurrent = Array.IndexOf(enumValues, EurekaHelper_DT.Config.BunnySoundEffect);
            if (ImGui.Combo("Bunny Sound Effect", ref enumCurrent, enumNames, enumNames.Length))
            {
                EurekaHelper_DT.Config.BunnySoundEffect = enumValues[enumCurrent];
                SoundManager.PlaySoundEffect(enumValues[enumCurrent]);
                save = true;
            }
            ...
            enumCurrent = Array.IndexOf(poValues, EurekaHelper_DT.Config.PayloadOptions);
            if (ImGui.Combo("Payload Options", ref enumCurrent, enumNames, enumNames.Length))
            {
                EurekaHelper_DT.Config.PayloadOptions = poValues[enumCurrent];
                save = true;
            }
            Utils.SetTooltip("Sets what the clickable payload does.\n" +
            ...
            enumCurrent = Array.IndexOf(chValues, EurekaHelper_DT.Config.ChatChannel);
            if (ImGui.Combo("Chat Channel", ref enumCurrent, enumNames, enumNames.Length))
            {
                EurekaHelper_DT.Config.ChatChannel = chValues[enumCurrent];
                save = true;
            }
            Utils.SetTooltip("Set the channel which the plugin messages will display. Default: Echo");
            ImGui.NextColumn();

            save |= ImGui.Checkbox("Randomize Map Coords", ref EurekaHelper_DT.Config.RandomizeMapCoords);
            Utils.SetTooltip("Randomizes map coords to range of +- 0.5 (recommended to enable)");
            ImGui.NextColumn();

            save |= ImGui.Checkbox("Auto Create Tracker", ref EurekaHelper_DT.Config.AutoCreateTracker);
            Utils.SetTooltip("Auto creates tracker when joining an instance and prints the tracker link to chat");
            ImGui.NextColumn();

            save |= ImGui.Checkbox("Auto Pop fate within range", ref EurekaHelper_DT.Config.AutoPopFateWithinRange);
            ...
            save |= ImGui.Checkbox("Show Level On Tracker", ref EurekaHelper_DT.Config.ShowLevelInTrackerTable);
            ...
            if (ImGui.CollapsingHeader("Custom Messages"))
            {
                ...
                if (ImGui.IsItemDeactivatedAfterEdit())
                {
                    ...
                    {
                        EurekaHelper_DT.Config.CustomMessages = CustomMessages.Split("\n").Where(x => !string.IsNullOrWhiteSpace(x)).ToList();
                        save = true;
                    }
                    else
                    {
                        CustomMessages = "/shout %bossName% POP. %flag%";
                        EurekaHelper_DT.Config.CustomMessages = new() { CustomMessages };
                        save = true;
                    }
                }
            }
        ...
        public static void DrawInstanceTab()
        {
            ...
            save |= ImGui.Checkbox("Display Server Id in chat", ref EurekaHelper_DT.Config.DisplayServerId);
            ImGui.NextColumn();

            save |= ImGui.Checkbox("Display Server Id in \"server info\" bar", ref EurekaHelper_DT.Config.DisplayServerIdInServerInfo);
